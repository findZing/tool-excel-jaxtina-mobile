const checkCumTuTrongCau = (cumTu, cau) => {
  ...
  if (
    !(cau_[index - 1]?.search(regex) > -1) &&
    !(cau_[index + cumTu_.length]?.search(regex) > -1)
  )
    {
      if(cau_[index - 1] && cau_[index - 1] === '/') return -1 //console.log("Before: ", cau_[index - 1], cau_[index - 1].length)
      return index;
    }
  return -1;
};
      ...
      for (let j = index; j < newSubEnglish.length; j++) {
        ...
        for (let z = j + dauCumTu - index; z < newWords[i].length; z++) {
          ...
          if (
            ...
            dauCumTu += 1;
            // console.log("Dau Cum Tu: ", newWords[i][j + dauCumTu - index - 1], newWords[i])
          }
        }

        if (
          j == newSubEnglish.length - 1 ||
          newSubEnglish[j + dauCau]?.toLowerCase() !==
            newWords[i]
              ?.replaceAll("’", "'")
              [j + dauCumTu - index]?.toLowerCase()
        ) {
          const tu = newSubEnglish.slice(
            index,
            j + (dauCau > dauCumTu ? dauCumTu : dauCau) + 1
          );
          ...
          newSubEnglish = newSubEnglish.replaceAll(
            tu,
            " _" + tu.replaceAll(" ", "_") + "_ "
          );
          break;
